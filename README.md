...
:다트: 사례 1: ParallelStream에서 forEach vs collect 비교
:열린_파일_폴더: 파일: StreamAPI4.java
:x: 비효율적인 코드
java
...
:다트: 사례 2: ParallelStream에서 Thread-Safe 컬렉션 사용
:열린_파일_폴더: 파일: StreamAPI3.java
:x: 비효율적인 코드 (주석 처리됨)
java
...
:전구: 왜 개선되었나?
ArrayList의 문제점멀티 스레드 환경에서 동시에 add() 호출 시 Race Condition 발생
데이터 손실 또는 ArrayIndexOutOfBoundsException 발생 가능
예상치 못한 결과 발생
CopyOnWriteArrayList의 장점쓰기 작업 시 내부 배열을 복사하여 Thread-Safe 보장
읽기 작업은 락 없이 수행 가능
병렬 처리 환경에서 안전하게 사용 가능
:경고: 주의사항
CopyOnWriteArrayList는 쓰기가 적고 읽기가 많을 때 유리
더 나은 방법: collect(Collectors.toList()) 사용 권장!
:다트: 사례 3: Stream 체이닝 최적화
:열린_파일_폴더: 파일: OverusingTest.java
:x: 비효율적인 코드 (test1)
java
...
:클립보드: 전체 성능 비교 (case03_compare.java)
실험 조건
작은 데이터: 5개 (출력 포함으로 병렬화 오버헤드 확인)
큰 데이터: 1,000,000개 (실제 성능 개선 확인)
핵심 발견
작은 데이터셋에서 parallelStream + println스레드 생성/관리 오버헤드가 더 큼
출력 작업의 동기화로 인한 성능 저하
큰 데이터셋에서 parallelStream + collect병렬 처리의 진정한 이점 발휘
효율적인 결과 수집
:반짝임: 결론 및 Best Practices
:일: ParallelStream 사용 시
:흰색_확인_표시: collect(Collectors.toList())를 사용하여 결과 수집
:x: forEach로 외부 컬렉션에 직접 추가하지 말 것
:둘: Thread-Safety
:흰색_확인_표시: 병렬 스트림에서는 Thread-Safe 컬렉션 사용
:흰색_확인_표시: 가능하면 collect()를 사용하여 안전하게 수집
:셋: Stream 체이닝
:흰색_확인_표시: 가능한 한 중간 연산을 통합
:흰색_확인_표시: 필터 조건은 &&로 결합
:흰색_확인_표시: 맵 변환은 하나의 람다로 처리
:넷: 성능 측정
